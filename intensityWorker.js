...
import { applyKunugi2012Filter } from "./jmaRealtimeFilter2012.js";
...
self.onmessage = (ev) => {
  ...
  const { requestId, stationCode, obsId, ns, ew, ud, sampleRateHz, recordTimeMs, windowSec, stepSec, filterType } = msg;

  try {
    const filterFn = filterType === "kunugi2012" ? applyKunugi2012Filter : applyJmaFilter;

    const nsFiltered = filterFn(ns, sampleRateHz);
    const ewFiltered = filterFn(ew, sampleRateHz);
    const udFiltered = filterFn(ud, sampleRateHz);
